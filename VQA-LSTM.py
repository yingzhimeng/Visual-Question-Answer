...
if num_hidlayer_lstm == 1:
    language_model.add(
        LSTM(output_dim=num_hidunit_lstm, return_sequences=False, input_shape=(max_len, word_vec_dim)))
else:
    language_model.add(
        LSTM(output_dim=num_hidunit_lstm, return_sequences=True, input_shape=(max_len, word_vec_dim)))
    for i in xrange(num_hidlayer_lstm - 2):
        language_model.add(LSTM(output_dim=num_hidunit_lstm, return_sequences=True))
    language_model.add(LSTM(output_dim=num_hidunit_lstm, return_sequences=False))
